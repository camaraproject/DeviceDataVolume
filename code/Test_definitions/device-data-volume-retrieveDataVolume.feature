...
# References to OAS spec schemas refer to schemas specifies in device-data-volume.yaml
  ...
  Background: Common retrieveDataVolume setup
    ...
    And the request body is compliant with the schema RetrieveDataVolumeRequest at "/components/schemas/RetrieveDataVolumeRequest"
...
#############Happy Path Scenarios##################	
  ...
  Scenario: Retrieve the estimated remaining data volume for a device
    ...
    When the HTTPS "POST" request retrieveDataVolume is sent
...
#############Error Response Scenarios##################
  ...
  Scenario: retrieve status request with invalid parameter 
    Given the request body is not compliant with the schema "/components/schemas/CheckDataVolumeRequest"
  ...
  @device_data_volume_04_expired_access_token
   Scenario: Expired access token
    Given a valid CheckDataVolumeRequest request body
  ...
  @device_data_volume_05_no_authorization_header
   Scenario: No Authorization header
    ...
    When the  request "CheckDataVolumeRequest " is sent
    Then the response status code is 401
    And the response property "$.status" is 401
    And the response property "$.code" is "UNAUTHENTICATED"
    And the response property "$.message" contains a user friendly text
